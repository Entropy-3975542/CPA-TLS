          ...
          scalar *tmp, EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
    ...
    uint8_t K[ML_KEM_SHARED_SECRET_BYTES];
    ...
    ret = hash_h(K, input, sizeof(input), mdctx, key)
        && encrypt_cpa(ctext, entropy, r, tmp, mdctx, key);
    OPENSSL_cleanse((void *)input, sizeof(input));

    if (ret)
        memcpy(secret, K, ML_KEM_SHARED_SECRET_BYTES);
    else
        ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR,
          ...
          EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
    ...
    if (!hash_h(K, decrypted, sizeof(decrypted), mdctx, key)) {
